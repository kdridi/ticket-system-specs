...
## Acceptance Criteria
- [ ] `specs.md` section 3.4 defines the roadmap format as YAML instead of markdown table
- [ ] The YAML format preserves all current fields: position, ticket ID, title, size, priority, dependencies, rationale
- [ ] `specs.md` section 4.2 updates all commands that read/write the roadmap (schedule, analyze, plan) to reference `roadmap.yml`
- [ ] `init-project.sh` creates `tickets/planned/roadmap.yml` with an empty list instead of `roadmap.md`
- [ ] The `ticket-system-conventions` skill is updated with the new roadmap format
- [ ] Insertion ordering logic (dependency-first, then priority) is clearly specified for the YAML structure
